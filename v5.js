...
function documentVisual(type,values={}){
  ...
  if(type==="BEFORE_AFTER")return `<div class="before-after-visual"><div><b>AVANT</b>${values.before_photo?`<img src="${values.before_photo}" alt="Situation avant">`:`<span>Photo avant</span>`}</div><div class="before-after-arrow">→</div><div><b>APRÈS</b>${values.after_photo?`<img src="${values.after_photo}" alt="Situation après">`:`<span>Photo après</span>`}</div></div>${values.description?`<p class="before-after-description">${esc(values.description)}</p>`:""}`;
...
function imageDataUrl(file){
  return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(new Error("Lecture impossible"));reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error("Image invalide"));img.onload=()=>{const max=1400,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement("canvas");canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL("image/jpeg",.78))};img.src=reader.result};reader.readAsDataURL(file)});
}
function beforeAfterForm(existing=null){
  const siteId=existing?.site_id||(state.site==="group"?"marzin":state.site),values=existing?.structured_data||{};
  modal(existing?`${existing.id} · Fiche Avant / Après`:"Nouvelle fiche Avant / Après",`<form id="beforeAfterForm" class="before-after-form"><div class="form-grid"><label class="wide">Titre<input id="baTitle" required value="${esc(existing?.title||"Amélioration · ")}" placeholder="Ex. Rangement des outils au poste 4"></label><label>Site<select id="baSite" ${existing?"disabled":""}>${OPERATIONAL_SITES.map(s=>`<option value="${s.id}" ${s.id===siteId?"selected":""}>${esc(s.name)}</option>`).join("")}</select></label><label>Responsable<input id="baOwner" value="${esc(existing?.owner||"")}" placeholder="Nom ou équipe"></label></div><div class="before-after-editor section"><label class="photo-drop"><b>PHOTO AVANT</b><span>Situation d’origine</span><div id="beforePreview" class="photo-preview">${values.before_photo?`<img src="${values.before_photo}" alt="Situation avant">`:`<strong>＋ Ajouter la photo</strong>`}</div><input type="file" id="beforePhoto" accept="image/*" capture="environment"></label><div class="before-after-arrow">→</div><label class="photo-drop"><b>PHOTO APRÈS</b><span>Amélioration réalisée</span><div id="afterPreview" class="photo-preview">${values.after_photo?`<img src="${values.after_photo}" alt="Situation après">`:`<strong>＋ Ajouter la photo</strong>`}</div><input type="file" id="afterPhoto" accept="image/*" capture="environment"></label></div><label class="wide">Descriptif court<textarea id="baDescription" rows="3" maxlength="500" placeholder="Qu’est-ce qui a changé et quel bénéfice constatez-vous ?">${esc(values.description||existing?.content||"")}</textarea></label><p class="hint">Les photos sont réduites automatiquement pour être conservées sur cet appareil.</p><div class="form-actions"><button class="btn">${existing?"Enregistrer":"Créer la fiche"}</button>${existing?'<button type="button" class="btn secondary" data-print>Imprimer / PDF</button>':""}<button type="button" class="btn ghost" data-close-modal>Annuler</button></div><div class="print-document">${documentVisual("BEFORE_AFTER",values)}</div></form>`);
  let beforePhoto=values.before_photo||"",afterPhoto=values.after_photo||"";
  const prepare=async(input,preview,setter)=>{const file=input.files?.[0];if(!file)return;try{const url=await imageDataUrl(file);setter(url);preview.innerHTML=`<img src="${url}" alt="Aperçu">`}catch(error){toast(error.message)}};
  $("beforePhoto").onchange=()=>prepare($("beforePhoto"),$("beforePreview"),value=>beforePhoto=value);
  $("afterPhoto").onchange=()=>prepare($("afterPhoto"),$("afterPreview"),value=>afterPhoto=value);
  $("beforeAfterForm").onsubmit=e=>{e.preventDefault();if(!beforePhoto||!afterPhoto){toast("Ajoutez une photo Avant et une photo Après.");return}const item=existing||{id:nextId("DOC",data.documents),type:"BEFORE_AFTER",status:"Brouillon",problem_id:null,version:1};item.site_id=existing?.site_id||$("baSite").value;item.title=$("baTitle").value.trim();item.owner=$("baOwner").value.trim();item.content=$("baDescription").value.trim();item.structured_data={before_photo:beforePhoto,after_photo:afterPhoto,description:item.content};item.updated_at=now();if(!existing)data.documents.unshift(item);try{save()}catch(error){toast("Photos trop volumineuses pour cet appareil.");return}state.site=item.site_id;closeModal();state.view="documents";render();toast(`Fiche ${item.id} enregistrée dans Documents → Mes documents.`)};
}
...
function editDocument(id,requestedStep=null){
  ...
  if(d.type==="BEFORE_AFTER")return beforeAfterForm(d);
  const schema=DOCUMENT_SCHEMAS[d.type]||DOCUMENT_SCHEMAS.ACTION_PLAN,problem=data.problems.find(p=>p.id===d.problem_id);
  if(!d.structured_data)d.structured_data=initialDocumentData(d.type,problem,d.content||"");
  const progress=documentProgress(d.type,d.structured_data),firstEmpty=schema.fields.findIndex(([field])=>!String(d.structured_data[field]||"").trim());
  const step=Math.max(0,Math.min(schema.fields.length-1,requestedStep??d.current_step??(firstEmpty<0?schema.fields.length-1:firstEmpty)));
  const [activeField,activeLabel,activeHint]=schema.fields[step],isLast=step===schema.fields.length-1;
  const stepper=schema.fields.map(([field,label],index)=>`<button type="button" class="wizard-step ${index===step?"active":""} ${String(d.structured_data[field]||"").trim()?"done":""}" data-doc-go="${index}" aria-label="Étape ${index+1} : ${esc(label)}"><b>${String(d.structured_data[field]||"").trim()?"✓":index+1}</b><span>${esc(label.replace(/^\d+\.\s*|^D\d\s*·\s*/,""))}</span></button>`).join("");
  modal(`${d.id} · ${schema.title}`,`<form id="editDocumentForm" class="structured-document document-wizard"><div class="document-toolbar"><div><p class="eyebrow">${esc(getSiteName(d.site_id))} · ${esc(d.type)} · version ${d.version||1}</p><h2>${esc(d.title)}</h2></div><div class="document-progress"><b>${progress.pct} %</b><span>${progress.done}/${progress.total} étapes terminées</span><div class="progress"><span style="width:${progress.pct}%"></span></div></div></div><div class="wizard-layout"><nav class="wizard-nav" aria-label="Étapes du document"><p>Votre parcours</p>${stepper}</nav><main class="wizard-main"><div class="wizard-position">ÉTAPE ${step+1} SUR ${schema.fields.length}</div><h2>${esc(activeLabel)}</h2><p class="wizard-question">${esc(activeHint)}</p><div class="wizard-example"><b>Exemple pour vous aider</b><span>${esc(documentExample(d.type,activeField))}</span></div><label class="wizard-answer">Votre réponse<textarea data-doc-field="${activeField}" rows="9" placeholder="Écrivez votre réponse ici…" autofocus>${esc(d.structured_data[activeField]||"")}</textarea></label><details class="wizard-details"><summary>Informations du dossier</summary><div class="form-grid document-meta"><label class="wide">Titre<input id="editDocumentTitle" value="${esc(d.title)}" required></label><label>Responsable<input id="editDocumentOwner" value="${esc(d.owner||problem?.owner||"")}" required></label><label>Statut<select id="editDocumentStatus">${["Brouillon","En révision","Validé"].map(x=>`<option ${x===d.status?"selected":""}>${x}</option>`).join("")}</select></label></div></details></main></div><div class="wizard-actions"><button type="button" class="btn secondary" data-doc-go="${step-1}" ${step===0?"disabled":""}>← Retour</button><span>La réponse est enregistrée lorsque vous continuez.</span><button type="button" class="btn" data-doc-go="${step+1}" ${isLast?'hidden':''}>Enregistrer et continuer →</button><button class="btn" ${isLast?'':'hidden'}>Terminer et enregistrer ✓</button></div><div class="form-actions wizard-secondary"><button type="button" class="btn ghost" data-print>Imprimer / PDF</button><button type="button" class="btn ghost" data-close-modal>Enregistrer et quitter</button></div><div class="print-document">${documentVisual(d.type,d.structured_data)}<div class="document-sections">${schema.fields.map(([field,label,hint],index)=>`<section class="document-step"><div class="step-number">${index+1}</div><div class="step-content"><label>${esc(label)}<span>${esc(hint)}</span><textarea data-print-field="${field}" rows="4">${esc(d.structured_data[field]||"")}</textarea></label></div></section>`).join("")}</div></div></form>`);
  const capture=()=>{const current=document.querySelector(`[data-doc-field="${activeField}"]`);if(current)d.structured_data[activeField]=current.value.trim();d.title=$("editDocumentTitle")?.value.trim()||d.title;d.owner=$("editDocumentOwner")?.value.trim()||d.owner;d.status=$("editDocumentStatus")?.value||d.status;d.content=d.structured_data[schema.fields[0][0]]||"";d.updated_at=now();if(problem){problem.owner=d.owner;syncProblemDocument(problem,d.type,d.structured_data);problem.updated_at=now()}};
  document.querySelectorAll("[data-doc-go]").forEach(button=>button.onclick=()=>{capture();const target=Number(button.dataset.docGo);d.current_step=Math.max(0,Math.min(schema.fields.length-1,target));save();editDocument(id,d.current_step);toast(`Étape ${step+1} enregistrée.`)});
  document.querySelectorAll("[data-close-modal]").forEach(button=>button.onclick=()=>{capture();d.current_step=step;save();closeModal();render();toast(`${d.id} enregistré. Reprise prévue à l’étape ${step+1}.`)});
  document.querySelectorAll("[data-print]").forEach(button=>button.onclick=()=>{capture();document.querySelectorAll("[data-print-field]").forEach(field=>field.value=d.structured_data[field.dataset.printField]||"");window.print()});
  $("editDocumentForm").onsubmit=e=>{e.preventDefault();capture();const p=documentProgress(d.type,d.structured_data);if(d.status==="Validé"&&p.pct<100)return toast("Il reste des étapes à compléter avant la validation.");d.version=(d.version||1)+1;d.current_step=firstEmpty<0?schema.fields.length-1:step;save();closeModal();render();toast(`${d.id} enregistré en version ${d.version} · avancement ${p.pct} %.`)};
}
