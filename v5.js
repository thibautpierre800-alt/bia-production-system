...
function startTool(moduleId){
  const target=state.site==="group"?"marzin":state.site;let run=(data.toolRuns||[]).find(r=>r.module_id===moduleId&&r.site_id===target);if(!run){run={id:nextId("RUN",data.toolRuns),site_id:target,module_id:moduleId,status:"En cours",checks:[],started_at:now(),updated_at:now()};data.toolRuns.push(run);save()}state.view="tools";state.toolId=moduleId;render();document.getElementById("toolChecklist")?.scrollIntoView?.({behavior:"smooth",block:"start"});toast("Démarche enregistrée dans Bibliothèque → Mes démarches. Suivez maintenant la checklist.");
}
