...
const ROLES={
  ...
  lean:{label:"Responsable Lean Groupe",scope:"group",nav:["home","pilotage","sqcdp","actions","terrain","audits","resolution","projects","practices","documents","tools","account","settings"]},
  director:{label:"Directeur de site",scope:"site",nav:["home","pilotage","sqcdp","actions","terrain","audits","resolution","projects","documents","tools","account"]},
  terrain:{label:"Chef d’équipe / Opérateur",scope:"workshop",nav:["home","sqcdp","actions","terrain","audits","documents","account"]}
};
const NAV=[
  ...
  {id:"audits",icon:"◎",label:"Audit Terrain · 50"},
  {id:"resolution",icon:"◇",label:"Résolution"},
  {id:"projects",icon:"◆",label:"Chantiers"},
  {id:"practices",icon:"✦",label:"Bonnes pratiques"},
  {id:"documents",icon:"▤",label:"Documents"},
  {id:"tools",icon:"▧",label:"Outils Lean"},
...
function loadData(){
  try{
    ...
    if(saved?.meta?.schema===6)return {...saved,projects:saved.projects||[],toolRuns:saved.toolRuns||[],practices:saved.practices||[],audits:saved.audits||[]};
...
function pilotageChart(series,{title,unit="%",direction="high",contextLabel=site().name}){
  ...
  return `<article class="panel indicator-chart"><div class="section-title"><div><h2>${esc(title)} · ${String(last).replace(".",",")} ${esc(unit)}</h2><p class="hint">${target==null?"Cible à définir":`Cible ${direction==="low"?"≤":"≥"} ${String(target).replace(".",",")} ${esc(unit)} · Écart ${gap>0?"+":""}${String(gap).replace(".",",")} pt`} · Tendance ${delta>0?"+":""}${String(delta).replace(".",",")} pt</p></div>${target==null?pill("À QUALIFIER","neutral"):pill(good?"Conforme":"Écart",good?"done":"open")}</div>${lineChart(series,`${title} · ${contextLabel}`)}<div class="row-top"><span class="source-label">${esc(series.source||"Source à définir")}</span><span class="row-meta">${esc(series.labels?.at(-1)||"Période à définir")}</span></div></article>`;
}
...
function renderAudits(){
  ...
  const detailed=rows.filter(a=>a.audit_data?.domain_scores),last=detailed[0],domains=last?Object.entries(last.audit_data.domain_scores):[];
  return `${pageHead("Audits","Audit Terrain BIA · 50 critères","Dix domaines, cinq critères par domaine et une preuve obligatoire pour chaque note.",'<button class="btn" data-new-audit>＋ DÉMARRER L’AUDIT 50 CRITÈRES</button>')}
    ...
    <section class="section"><div class="section-title"><h2>Historique</h2><button class="btn secondary" data-open-tool="audit-terrain">Ouvrir le guide Audit Terrain</button></div><div class="table-wrap"><table class="data-table"><thead><tr><th>ID</th><th>Type</th><th>Périmètre</th><th>Score</th><th>Preuves</th><th>Date</th><th>Responsable</th></tr></thead><tbody>${rows.map(a=>`<tr><td>${esc(a.id)}</td><td>${esc(a.type)}</td><td>${esc(a.scope)}</td><td><b>${a.score}/100</b></td><td>${a.audit_data?.proof_count??(a.proof?1:"—")}</td><td>${shortDate(a.performed_at)}</td><td>${esc(a.owner)}</td></tr>`).join("")||'<tr><td colspan="7">Aucun audit.</td></tr>'}</tbody></table></div></section>`;
}
...
function auditForm(){
  const siteId=state.site==="group"?"marzin":state.site,auditSites=canGroup()?OPERATIONAL_SITES:OPERATIONAL_SITES.filter(s=>s.id===siteId);
  modal("Audit Terrain BIA · 50 critères",`<form id="auditForm"><div class="form-grid"><label>Site<select id="auditSite">${auditSites.map(s=>`<option value="${s.id}" ${s.id===siteId?"selected":""}>${esc(s.name)}</option>`).join("")}</select></label><label>Périmètre<input id="auditScope" required placeholder="Atelier / zone / processus"></label><label>Responsable du périmètre<input id="auditOwner" required></label><label>Auditeur<input id="auditAuditor" required></label><label>Date<input id="auditDate" type="date" value="${today()}" required></label><label>Type<select id="auditType"><option>Audit Terrain BIA</option><option>Audit 5S détaillé</option><option>Audit de maturité</option><option>Audit standard</option></select></label></div>
    <div class="alert section"><b>Notation :</b> 0 inexistant · 1 très faible · 2 insuffisant · 3 acceptable · 4 maîtrisé · 5 excellence. Une preuve est obligatoire pour chaque critère.</div>
    <div class="audit-form section">${Object.entries(AUDIT_CRITERIA).map(([domain,criteria],di)=>`<fieldset class="audit-fieldset"><legend>${di+1}. ${esc(domain)}</legend>${criteria.map((criterion,ci)=>`<div class="audit-criterion"><div><b>${di+1}.${ci+1} ${esc(criterion)}</b></div><label>Note<select data-audit-score data-domain="${esc(domain)}" data-criterion="${esc(criterion)}" required><option value="">—</option>${[0,1,2,3,4,5].map(n=>`<option value="${n}">${n}</option>`).join("")}</select></label><label>Preuve / observation<input data-audit-proof required placeholder="Fait observé, document ou absence constatée"></label></div>`).join("")}</fieldset>`).join("")}</div>
    <label class="field section">Synthèse et priorité<textarea id="auditSummary" rows="4" required></textarea></label><label class="check section"><input id="auditAction" type="checkbox" checked> Créer une action pour chaque domaine dont la moyenne est inférieure à 3/5</label><div class="form-actions"><button class="btn">Enregistrer l’Audit Terrain</button><button type="button" class="btn secondary" data-close-modal>Annuler</button></div></form>`);
  $("auditForm").onsubmit=e=>{e.preventDefault();const scores=[...document.querySelectorAll("[data-audit-score]")],proofs=[...document.querySelectorAll("[data-audit-proof]")];if(scores.some(x=>x.value==="")||proofs.some(x=>!x.value.trim()))return toast("Chaque critère doit avoir une note et une preuve.");const id=nextId("AUD",data.audits),site=$("auditSite").value,domainScores={};for(const domain of Object.keys(AUDIT_CRITERIA)){const values=scores.filter(x=>x.dataset.domain===domain).map(x=>Number(x.value));domainScores[domain]=values.reduce((a,b)=>a+b,0)/values.length}const average=scores.reduce((sum,x)=>sum+Number(x.value),0)/scores.length,score=Math.round(average/5*100),summary=$("auditSummary").value.trim();data.audits.unshift({id,site_id:site,type:$("auditType").value,scope:$("auditScope").value.trim(),score,status:"Terminé",performed_at:$("auditDate").value,owner:$("auditOwner").value.trim(),audit_data:{auditor:$("auditAuditor").value.trim(),summary,domain_scores:domainScores,criteria:scores.map((x,i)=>({domain:x.dataset.domain,criterion:x.dataset.criterion,score:Number(x.value),proof:proofs[i].value.trim()})),proof_count:proofs.length}});if($("auditAction").checked)Object.entries(domainScores).filter(([,value])=>value<3).forEach(([domain,value])=>data.actions.unshift({id:nextId("A",data.actions),site_id:site,workshop_id:data.workshops.find(w=>w.site_id===site)?.id||null,title:`Audit · Renforcer ${domain}`,owner:$("auditOwner").value.trim(),priority:value<2?"Haute":"Normale",status:"Ouverte",due_date:null,origin_type:"Audit",origin_id:id,description:`${summary} · moyenne ${value.toFixed(1)}/5`,effectiveness:null,created_at:now()}));save();closeModal();state.site=site;render();toast(`Audit ${id} enregistré : ${score}/100 et preuves conservées.`)};
}
...
function bind(){
  ...
  document.querySelectorAll("[data-open-tool]").forEach(b=>b.onclick=()=>{state.toolId=b.dataset.openTool;state.view="tools";render()});
