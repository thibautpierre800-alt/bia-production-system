...
function simpleDocument(type){
  ...
  $("documentForm").onsubmit=e=>{e.preventDefault();const item={id:nextId("DOC",data.documents),site_id:$("documentSite").value,type,title:$("documentTitle").value.trim(),status:"Brouillon",problem_id:null,owner:$("documentOwner").value.trim(),content:$("documentContext").value.trim(),updated_at:now(),version:1};data.documents.unshift(item);save();closeModal();state.site=item.site_id;render();toast(`Document ${item.id} créé. Retrouvez-le dans Documents → Mes documents.`)}
}
function editDocument(id){
  const d=data.documents.find(x=>x.id===id);if(!d)return;
  modal(`${d.id} · ${d.title}`,`<form id="editDocumentForm"><div class="form-grid"><label class="wide">Titre<input id="editDocumentTitle" value="${esc(d.title)}" required></label><label>Responsable<input id="editDocumentOwner" value="${esc(d.owner||"")}"></label><label>Statut<select id="editDocumentStatus">${["Brouillon","En révision","Validé"].map(x=>`<option ${x===d.status?"selected":""}>${x}</option>`).join("")}</select></label><label class="wide">Contenu / contexte<textarea id="editDocumentContent" rows="10" placeholder="Renseigner le document">${esc(d.content||"")}</textarea></label></div><div class="alert section"><b>Traçabilité :</b> ${esc(getSiteName(d.site_id))} · version ${d.version||1}${d.problem_id?` · problème lié ${esc(d.problem_id)}`:""}</div><div class="form-actions"><button class="btn">Enregistrer une nouvelle version</button><button type="button" class="btn secondary" data-print>Imprimer / PDF</button><button type="button" class="btn ghost" data-close-modal>Fermer</button></div></form>`);
  $("editDocumentForm").onsubmit=e=>{e.preventDefault();d.title=$("editDocumentTitle").value.trim();d.owner=$("editDocumentOwner").value.trim();d.status=$("editDocumentStatus").value;d.content=$("editDocumentContent").value.trim();d.version=(d.version||1)+1;d.updated_at=now();save();closeModal();render();toast(`${d.id} enregistré en version ${d.version}. Retrouvez-le dans Documents → Mes documents.`)};
}
